...
fn integrate_scattered_luminance(world_pos: vec3<f32>, world_dir: vec3<f32>, sun_dir: vec3<f32>, atmosphere: Atmosphere) -> IntegrationResults {
	...
	for (var s: f32 = 0.0; s < sample_count; s += 1.0) {
		...
		let earth_shadow = f32(!ray_intersects_sphere(sample_pos, sun_dir, planet_center + planet_radius_offset * zenith, atmosphere.bottom_radius));
        let scat = earth_shadow * transmittance_to_sun * (medium.scattering * isotropic_phase);
